...
namespace DontShaveYourHead
{
	...
	{
		const float YOffset_OnHead = 0.03030303f; //value comes from Assembly-CSharp.Verse.PawnRenderer.YOffset_OnHead
		const float YOffset_PostHead = 0.03409091f; //value comes from Assembly-CSharp.Verse.PawnRenderer.YOffset_PostHead
		...
		public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
		{
			...
			foreach (var code in instructions)
			{
				...
				else if (code.opcode == OpCodes.Ldloc_S && code.operand is LocalBuilder b && b.LocalIndex == 12 /* the flag is the 12th in the LocalIndex */)
				{
				...
				else if (afterHairMatAt && code.opcode == OpCodes.Ldloc_S && code.operand is LocalBuilder b2 && b2.LocalIndex == 11)
				{
					//loading loc1 and subtracting the offset from loc1's y field
					yield return new CodeInstruction(OpCodes.Ldloca_S, 11); //load loc1
				...
				else if (afterHairMatAt && code.opcode == OpCodes.Call && (MethodInfo)code.operand == AccessTools.Method(typeof(GenDraw), nameof(GenDraw.DrawMeshNowOrLater)))
				{
